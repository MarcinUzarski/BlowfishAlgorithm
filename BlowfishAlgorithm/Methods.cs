...
namespace BlowfishAlgorithm
{
    ...
    {
        static string[] P = new string[]{ "243f6a88", "85a308d3", "13198a2e", "03707344", "a4093822",
        ...
        private const long modVal = 4294967296;   // wartość wyrażenia 2^32

        public static string ConvertKey(string key)
        {
            var newKey = "";
            int m = 1;
            while (newKey.Length <= 448)
            {
                newKey = string.Concat(Enumerable.Repeat(key, m));
                m++;
            }
            newKey = newKey.Substring(0, 448);
            byte[] key_array = Encoding.ASCII.GetBytes(newKey);
                string binValues = "";
                for (int i = 0; i < key_array.Length; i++)
                {
                    binValues += Convert.ToString(key_array[i], 2).PadLeft(8, '0');
                }

                return hexConvert(binValues);
        }


        public static string ConvertInputText(string inputText)
        {
            byte[] key_array = Encoding.ASCII.GetBytes(inputText);
            string binValues = "";
            for (int i = 0; i < key_array.Length; i++)
            {
                binValues += Convert.ToString(key_array[i], 2).PadLeft(8, '0');
        ...
        public static void generateKey(String key)
        {
            ...
            {
                P[i] = xor(P[i], key.Substring(j, 8));
                j = (j + 8) % key.Length;
            }
        }

        public static string hexConvert(string strBinary)
        {
            var hex = string.Join("",
                Enumerable.Range(0, strBinary.Length / 4)
                    .Select(i => Convert.ToByte(strBinary
                        .Substring(i * 4, 4), 2).ToString("X")));
            return hex;
        ...
        public static string xor(string a, string b)
        {
            ...
            return hexConvert(ans);
        }

        public static string txtToBin(string inputText)
        {
            string binary = String.Join(String.Empty,
                inputText.Select(
                    i => Convert.ToString(
                        Convert.ToInt32(i.ToString(), 16), 2)
                        .PadLeft(4, '0')
                    )
            );
        ...
        public static string encrypt(string plainText)
        {
            ...
            string left = plainText.Substring(plainText.Length/2, plainText.Length/2);
        ...
        public static string round(int index, string plainText)
        {
            string right = plainText.Substring(0, plainText.Length/2);
            string left = plainText.Substring(plainText.Length/2, plainText.Length/2);
            left = xor(left, P[index]);
            string functionFOut = f(left);
            right = xor(functionFOut, right);
		...
		private static string f(string plainText)
		{
			...
			for (int i = 0; i < 8; i += 2)
            {
                int col = Convert.ToInt32(txtToBin(plainText.Substring(i, 2)), 2);
				a[i / 2] = S[i / 2, col];
			}
			ans = addBin(a[0], a[1]);
			ans = xor(ans, a[2]);
        ...
        {
            ulong n1 = Convert.ToUInt64(a, 16);
            ulong n2 = Convert.ToUInt64(b, 16);
            n1 = (n1 + n2) % modVal;
            return n1.ToString("X").PadLeft(8,'0');
        }
